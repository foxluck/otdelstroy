        ...
        foreach ($object->__typedef as $typeName => $typeValue) {
            ...
            if (is_array($typeValue)) {
                ...
                if (is_array(current($typeValue)) && count($typeValue) == 1
                    ...
                    $thisType['type'] = 'Array';
                    $currentTypeValue = current($typeValue);
                    reset($currentTypeValue); //?? Only variables should be passed by reference File:WSDL.php Line: 1909 Error #: 2048 ??
                    list($nsPrefix, $typeName) = $this->_getTypeNs(current($currentTypeValue));
