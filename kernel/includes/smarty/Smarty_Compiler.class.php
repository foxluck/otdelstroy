...
class Smarty_Compiler extends Smarty {
    ...
    function _compile_file($resource_name, $source_content, &$compiled_content)
    {
        ...
        $safe_resource_name = strtr(urlencode($resource_name), array('%2F'=>'/', '%3A'=>':'));
        $template_header = "<?php /* Smarty version ".$this->_version.", created on ".strftime("%Y-%m-%d %H:%M:%S")."\n";
        $template_header .= "         compiled from ".$safe_resource_name." */ ?>\n";
        
        if(defined('SMARTY_DEBUG_ENABLE')&&constant('SMARTY_DEBUG_ENABLE')){//debug futures for SC
        	$template_header .="\n<!-- Begin {$safe_resource_name} -->\n";
        	$template_header .="<?php\n";
        	$template_header .="if(class_exists('Debug')){\n\t\$debuger = Debug::getInstance('smarty');\n\t\$debuger->start();\n}";
        	$template_header .="\n?>\n";
        }
        ...
        $template_footer = '';
        if(defined('SMARTY_DEBUG_ENABLE')&&constant('SMARTY_DEBUG_ENABLE')){ //debug futures for SC
        	$template_footer .="\n<!-- End {$safe_resource_name} -->\n";       
        	$template_footer .="\n<?php\n";
        	$template_footer .='if(class_exists(\'Debug\')){
        		$debuger->end(array(\'path\'=>\''.$safe_resource_name.'\'),\'smarty\');
        	}';
        	$template_footer .="\n?>\n";
        }

        $compiled_content = $template_header . $compiled_content.$template_footer;
        return true;
    }
