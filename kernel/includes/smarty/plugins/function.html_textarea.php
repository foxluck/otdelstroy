	...
	function smarty_function_html_textarea($params, &$smarty){
		...
		foreach ($languages as $languageEntry){
			...
			<tr>
				...
				':'').'
				<td>
					<textarea lang="'.$lang_iso2.'" name="'.xHtmlSpecialChars($input_name).'" '.(isset($id)?" id='{$id}_{$lang_iso2}'":'').(isset($rows)?" rows='{$rows}'":'').(isset($cols)?" cols='{$cols}'":'').(isset($style)?" style='{$style}'":'').(isset($class)?" class='{$class}'":'').'>'.$values[$dbfield.'_'.$lang_iso2].'</textarea>
				</td>
			</tr>';
		...
		<tr>
			...
			<td valign="bottom" '.(isset($style)&&preg_match('@width:\s?100\%@', $style)?' style="width: 1%;"':'').'><a title="'.xHtmlSpecialChars(translate('loc_show_empty_translations')).'" href="#expand" class="expand_languages" rel="'.xHtmlSpecialChars($id).'"><img style="margin: auto 4px;" src="./images_common/show_other_languages.gif" alt="'.xHtmlSpecialChars(translate('loc_show_empty_translations')).'" width="11" height="11"></a>
			':'').'
			</td>
		</tr>
		</table>
