    ...
    'http://xml.apache.org/xml-soap' => array('Map')
		...
		function serialize_val($val,$name=false,$type=false,$name_ns=false,$type_ns=false,$attributes=false,$use='encoded',$soapval=false) {
			...
			if (is_object($val)) {
				...
				$this->debug("serialize_val: object to array");
				$val = (array)$val;
				$this->debug("serialize_val of soapval returning ".var_export($val,true));
			}
...
class nusoap_server extends nusoap_base {
	...
	function invoke_method() {
		...
		if ($this->wsdl) {
			$soap_version = ($this->wsdl->namespaces&&isset($this->wsdl->namespaces['soap12']))?'soap12':'soap';
			if ($this->opData = $this->wsdl->getOperationData($this->methodname,$soap_version)) {
		...
		if (strlen($delim) > 0 && substr_count($this->methodname, $delim) == 1) {
			...
			$this->debug("in invoke_method, method=$method");
		}else {
			$try_class = '';
		...
		if($this->handler) {
			...
			} else if(class_exists($this->handler)) {
				$class = $this->handler;
				$handler_class = $class;
			} else {
				$this->debug("in invoke_method, got invalid handler!");
				$this->result = 'fault: method not found';
				$this->fault('SOAP-ENV:Client',"method '$this->methodname'('$orig_methodname') not defined in service('$try_class')");
				return;
			}
			$method_to_compare = (substr(phpversion(), 0, 2) == '4.') ? strtolower($method) : $method;
			if (!in_array($method_to_compare, get_class_methods($handler_class))) {
				$this->debug("in invoke_method, method '$this->methodname' not found in class '$handler_class'!");
				$this->result = 'fault: method not found';
				$this->fault('SOAP-ENV:Client',"method '$this->methodname'/'$method_to_compare'('$orig_methodname') not defined in service/'$class'('$handler_class' '$delim')");
				return;
			}
		} else if ($class == '') {
		...
		if (!function_exists('call_user_func_array')) {
			if ($class == '') {
				...
				$funcCall = "\$this->methodreturn = \$this->handler->{$this->methodname}(";
			} else {
				if ($delim == '..') {
			...
			if ($this->handler && $class == '') { 
				$this->debug('in invoke_method, calling handler method using call_user_func_array()');
				$call_arg = array(&$this->handler, $method);
			} elseif ($class == '') {
	...
	function serialize_return() {
		...
		if (isset($this->methodreturn) && is_object($this->methodreturn) && ((get_class($this->methodreturn) == 'soap_fault') || (get_class($this->methodreturn) == 'nusoap_fault'))) {
			...
			if($this->wsdl){
				...
				$soap_version = ($this->wsdl->namespaces&&isset($this->wsdl->namespaces['soap12']))?'soap12':'soap';
				$return_val = $this->wsdl->serializeRPCParameters($this->methodname,'output',$opParams,$soap_version);
				$this->appendDebug($this->wsdl->getDebug());
				$this->wsdl->clearDebug();
	...
	function verify_method($operation,$request){
		if(isset($this->wsdl) && is_object($this->wsdl)){
			$soap_version = ($this->wsdl->namespaces&&isset($this->wsdl->namespaces['soap12']))?'soap12':'soap';
			if($this->wsdl->getOperationData($operation,$soap_version)){
				return true;
			}
				...
				if(is_array($msgParts)){
					foreach($msgParts as $partName => $partType) {
						...
						if (strpos($partType, ':')) {
							...
							if (!isset($typePrefix)) {
								throw new Exception("$partType has no namespace!");
							}
						}
