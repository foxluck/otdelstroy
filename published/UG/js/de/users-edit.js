...
function AddInput(obj) {
	$('<input type = "text" /><a class="delete_email" href="javascript:void(0)" onClick="DeleteInput(this)">Delete</a><br />').insertBefore(obj);
...
var WbsEditUser = newClass(null, {
	...
	prepareGroup: function(group) {
		...
		var links = $('<div class="field"><div class="label">&nbsp;</div></div>');
		var buttons = $('<div class="field_controls"></div>');
		var save = $('<input type="button" value="Save" />').click(function(){this.save(group);}.bind(this));
		var cancel = $('<input type="button" value="Cancel" />').click(function () {
			group.edit = false;
		...
		if (f.type == 'IMAGE') {
			...
			if (f.id == 'CF_ID') {
				if (document.isContactAdmin) {
					edit.append('<option value="">&lt;none folder&gt;</option>');
				}
			} else if (!this.config.noSelect && !f.required) { 
				edit.append('<option value="">&lt;select&gt;</option>');
			}
			...
			for (i = 1; i < f.value.length; i++) {
				ss += '<input type = "text" /><a class="delete_email" href="javascript:void(0)" onClick="DeleteInput(this)">Delete</a><br />';
			}
			ss += '<a onfocus="blur(this)" href="javascript:void(0)" class="add_email" onClick="AddInput(this)">Add another</a>';
			edit = ss;
		} else {
		...
		if (f.max_length != undefined && f.max_length) {
			if (edit.blur) {
				edit.blur(function () {
					...
					if (l > f.max_length) {
						_this.showFieldError({id: f.id, text: 'Number of characters can not exceed ' + f.max_length + '.'});
					}
				});
			}
		}
	...
	setValue: function (g, f) {
		...
		if (f.type == 'TEXT') {
			...
			if (f.id == 'CF_ID' && !v) {
				$("#" + f.id + " > div.edit").html('&lt;none folder&gt;');
			} else {
			...
			if (f.id == this.config.photoField) {
				...
				if (v && v != null && v.length > 0) {
					...
					div.append($('<p style="width:105px;text-align:center"><a href="javascript:void(0)">Change photo</a></p>')
					.click(function(){
						$('#popup2').wbspopup({
					...
					div.append($('<p class="add-photo"><a href="javascript:void(0)">Add photo</a></p>'))
					.click(function(){
						$('#popup2').wbspopup({
			...
			if (v && v != null && v.length > 0) {			
				...
				if (div.children().length == 1) {
					var del = $('<a id="delete_' + f.id + '" href="#">Delete</a>').click(function () {
							this.deleteImage(g, f);
					}.bind(this));
					var replace = $('<a class="upload" id="upload_' + f.id + '" href="#">Replace</a>');
					if (f.id == this.config.photoField) {
						div.append($('<p style="width:100px;text-align:center"><a href="javascript:void(0)">Replace</a></p>'));
					} else {
				...
				var div = $("#edit_" + f.id + " > div.edit");
				if (div.children().length == 1) {
					div.append($('<p style="padding-top:0" align="center"><a class="upload" id="upload_' + f.id + '" href="#">Upload image</a></p>'));
				} else {
					div.find("p").replaceWith($('<p align="center"><a class="upload" id="upload_' + f.id + '" href="#">Upload image</a></p>'));
				}
				if ($("#edit_" + f.id).is(":visible")) {
					this.setUpload(g, f);
				}
			...
			if ((!v || v == '0') && this.config.emptyValue) {
				...
				for (var k = 0; k < v.length; k++) {
					...
					if (this.config.compose_mail) {
						h += ' <a class="compose-message" href="?mod=users&act=email&mode=contacts&contact_id=' + this.config.contact + '&email=' + v[k] + '">Compose message</a>';
					}
					h += '</div>';
				}
			}
		...
		// MOBILE
			...
			if (v) {
				var a = $('<a class="send-sms" href="javascript:void(0)" title="Send SMS">Send SMS</a>');
				a.click(function () {
					$('#popup').wbsPopup({
				       width: 600,
				       height: 'auto',
				       ...
				       loadComplite: function () {
							...
							$('#popup').prepend(
								'<div class="wbs-dlg-header">'+
									'<div class="label">Send SMS</div>'+
									'<div class="close-btn"><img width="16" height="16" src="../common/templates/img/close.gif" /></div>'+
								'</div>'
							);
							$('#popup').append(
								'<div class="wbs-dlg-footer">'+
									'<input id="doSendsmsBtn" class="wbs-dlg-button" type="button" value="Send"/>'+
									'<input class="wbs-dlg-button" type="button" value="Cancel" onclick="$(\'#popup\').wbsPopupClose()" />'+
								'</div>'
							);
							$("#To").val(v);
							$('#doSendsmsBtn').click(function(){
								...
								if (!$("#To").val().replace(/^\s*(.*?)\s*$/,"$1")) {
									$('#error_block').html("Please enter phone number").show();
									$('#To').focus();
								} else if (!$("#message").val().replace(/^\s*(.*?)\s*$/,"$1")) {
									$('#error_block').html("Please enter message text");
									$('#error_block').show();
									$('#message').focus();
								} else {
										...
										function (response) {
											try {
												...
												if (r.Error) {
													...
													$("#onload-message").show().html('SMS has been sent successfully');
	...
	showError: function(error, group) {
		...
		if (this.editAll) {
			...
			if ($(id + " div.error").length) {
				...
				$(id).prepend('<div class="error">Please correct errors.</div>');
			}
		}
	},
	...
	setEditAll: function(button) {
		...
		if (this.editAll) {
			...
			var save = $('<input id="save_all" type="button" value="Save" />').css('margin-right', '5px').click(function () {
				this.save(null);
			...
			var cancel = $('<input id="cancel_all" type="button" value="Cancel" />').click(function () {
