...
function renderApps(apps, is_group) {
	...
	if (!is_group) {
		t.append('<tr class="first"><td style="border:0; height: 28px">&nbsp;</td><td class="right" width="150">[`Personal access rights`]</td><td class="right" width="150">[`Inherited from groups`]</td></tr>');
	}
	for (var i = 0; i < apps.length; i++) {
		if (apps[i][0] == 'UG' && self_rights) {
			var select_html = '[`administrator`]';
		} else {
			var select_html = '<select name="access' + apps[i][0] +  '">' +
			'<option value="0"' + (!apps[i][3][1] ? " selected" : "") + '>[`no access`]</option>' +
			(hasAdmin(apps[i][0]) ? '<option value="7"' + (apps[i][3][1] == 7 ? " selected" : "") + '>[`administrator`]</option>' : '') +
			(apps[i][4].length >= 1 || apps[i][5].title ? '<option value="1"' + (apps[i][3][1] == 1 || (apps[i][3][1] && !hasAdmin(apps[i][0]))  ? " selected" : "") + '>[`custom`]</option>' : '') + 
			'</select>';
		}
		t.append('<tr class="title' + (apps[i][3][0] ? ' access-title' : '' ) + '"><td class="title" >' + apps[i][1] + '</td>' + 
				...
				(apps[i][3][2] == 7 ? '[`administrator`]' : (apps[i][3][2] == 1 ? '[`custom`]' : '[`no access`]')) + 
				'</td>') + '</tr>');
		...
		for (var j = 0; j < rights.length; j++) {
			...
			if (section[0] == 'QUOTA') {
				...
				var app_id = apps[i][0];
				var a = $('<a class="quota" href="#">' + (section[3] ? section[3] + ' [`KB`]' : '[`unlimited`]') + '</a>').click(function () {
					...
					div.append($('<input type="button" value="[`Cancel`]" />').click(function () {
						$(this).parent().remove();
					}));					
						 ...
						 {action: 'save', 
						  ...
						  function (reposnse) {
							...
							if ($("#onload-message.rr").length == 0) {
								var message = '[`All changes are automatically applied`]';
		...
		if (apps[i][5].title) {
			t.append('<tr class="section ' + apps[i][0] + (!apps[i][3][0] || apps[i][3][0] == '7' ? " dn" : "") + '"><td class="title comment"><b>' + apps[i][5].title + '</b>' + '</td><td align="center" width="150"><table width="150"><tr><td width="33%">[`Read`]</td><td width="34%">[`Write`]</td><td width="33%">[`Full`]</td></tr><tr><td colspan="3" class="all"><input type="checkbox" value="1" /><input type="checkbox" value="3" /><input type="checkbox" value="7" /></td></tr></table></td><td></td></tr>');
	...
	for (var i = 0; i < apps.length; i++) {
		...
		(function () {
			...
			$("td#screen" + apps[n][0] + " > select").change(function () {
				$(this).hide();
				$(this).parent().append('<div class="loading"><img style="vertical-align:middle" width="16" height="16" src="' + WbsData.get('url.common') + 'templates/img/loading.gif" /> [`Loading`]...</div>');
				var r = $(this).val();
				...
				if (r == '0') {
					$.post("index.php?mod=" + (is_group ? "groups" : "users") + "&act=rights&ajax=1", {action: 'del', id: right_id, application_id: apps[n][0]}, function (response) {
						...
						if (str.length > 0) {
							...
							$("#access-note").show();
							$("#user-apps").html('<span style="color:#666">[`none`]</span>');
						}
