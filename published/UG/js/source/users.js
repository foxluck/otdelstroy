var UGApplication = newClass(WbsApplication, {
	...
	init: function() {	
		this.resize();	
		this.table.pager.config.nameElements = '[`Users`]';
...
var UGUsersListMenu = newClass(WbsPopmenu, {
	constructor: function(recordsList) {
		...
		config.items = config.items.concat([
			{label: "[`Actions with selected users`] (" + recordsList.getCount() + '):' , cls: "unactive"},
			"-",
			{label: "[`Disable login`]", disabled: disabled, onClick: recordsList.setDisabled.bind(recordsList)},
			{label: "[`Enable login`]", disabled: disabled, onClick: recordsList.setEnabled.bind(recordsList)},
			"-",
			{label: "[`Delete users`]", disabled: disabled, onClick: recordsList.tryDelete.bind(recordsList)}
		]);
		this.group = document.app.getCurrentFolder();
		if (this.group.Id > 0) {
			config.items = config.items.concat([
			    {label: "[`Action with group:`]", cls: "unactive"},
				{label: "[`Rename`]", disabled: this.group.Id <= 0, onClick: this.group.showRename.bind(this.group)}, 
				{label: "[`Edit membership and access rights`]", disabled: this.group.Id <= 0, onClick: function() {document.app.openSubframe('index.php?mod=groups&act=edit&id='+this.group.Id, true);}},
				{label: "[`Delete`]", disabled: this.group.Id <= 0, onClick: this.group.tryDelete.bind(this.group)}
			]);
		}
