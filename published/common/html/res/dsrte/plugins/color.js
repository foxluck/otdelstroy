...
var dsRTE_color = function() {
    ...
    this.ExecuteCommand = function( dsrte, arguments, panel ) {
		...
		var id = (arguments == 'forecolor') ? 'fgcolor' : 'bgcolor';
		var pos = $('#cmd-' + dsrte.iframe.id + '-' + id + ' span').position().left;
		$('#' + dsrte.iframe.id + '-color').width(78);
		$('#' + dsrte.iframe.id + '-color').css('left', pos + 'px');
    ...
    this.PrepareCommand = function( dsrte, arguments, panel, $this ) {
        ...
        if ( $( id ).attr( 'handled' ) != '1' ) {
            $( id ).attr( 'handled', '1' ).find( 'td' ).click(function() {
                self.ApplyColor( dsrte, $(this).attr( 'bgcolor' ) );
