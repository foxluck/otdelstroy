...
abstract class dsRTECommand
{
    ...
    protected function __construct( $id, $command, $arguments, $title, $icon_offset )
    {
        ...
        $this->title = $title;
        $this->icon_offset = is_numeric( $icon_offset ) ? 23 * $icon_offset : $icon_offset;
...
final class dsRTECommandSuperSeparator extends dsRTECommand
{
    /**
     * Return HTML for a block separator.
     */
    public function getHTML()
    {
        return '<div class="supersep"></div>';
    }
}

final class dsRTECommandLeftSeparator extends dsRTECommand
{
    /**
     * Return HTML for a block separator.
     */
    public function getHTML()
    {
        return '<div class="leftsep"></div>';
    }
}

final class dsRTECommandRightSeparator extends dsRTECommand
{
    /**
     * Return HTML for a block separator.
     */
...
class dsRTECommandDivSelect extends dsRTECommand
{
    /**
     * Get HTML code for a combobox.
     */
    public function getHTML()
    {
        $html = '<a id="cmd-'.$this->id.'-'.$this->command.'" class="cmd list" title="'.$this->title.'">';
        ...
        $html .= '</a>';
        $html .= '<div id="cmd-'.$this->id.'-'.$this->command.'-list" class="panel"><ul class="select">';

		$fontsize = array(2=>t('Small'), 3=>t('Medium'), 5=>t('Big'), 7=>t('Large'));

        foreach ($this->arguments as $item) {
			switch($this->command) {
				case 'fontname':
					$html .= '<li><a class="cmd" cmd="'.$this->command.'" title="'.$item.'"'
						.' style="font-family:'.$item.'">'.$item.'</a></li>';
					break;
				case 'fontsize':
					$html .= '<li><a class="cmd" cmd="'.$this->command.'" title="'.$item.'"'
					.' style="font-size:'.($item * 6).'px">'.$fontsize[$item].'</a></li>';
					break;
				default:
					$html .= '<li><a class="cmd" cmd="'.$this->command.'" title="'.$item.'">'.$item.'</a></li>';
			}
        }
        $html .= '</ul></div>';
...
class dsRTETextButton extends dsRTECommand
{
    /**
     * Return a command icon either by offset into the icons.gif image or as an external icon image.
     */
    public function getHTML()
    {
        $html = '<a style="float:right" class="cmd" id="cmd-'.$this->id.'-'.$this->command.'" title="'.$this->title.'">';
        $html .= '<span style="background:none; border: 0; width: auto; padding-top: 5px">'.$this->title.'</span>';
        $html .= '</a>';
        return $html;
    }
}
