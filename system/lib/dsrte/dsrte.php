...
define('DSRTE_DIR', dirname(__FILE__));

// Base classes
require_once DSRTE_DIR . '/dsrte.command.php';
...
class dsRTE extends dsRTECommand
{
     ...
     */
    public function __construct( $id, $str = false, $resizable = false )
    {
        $this->id = $id;
        ...
        $cmds = $str !== false ? $str : '<| font | fontsize || bold | italic | underline | strikeout || color | bgcolor || left | center | right || ul | ol || indent | outdent || link | image | rule  || source || insertvar |>';
            ...
            foreach ( explode( ' ', $cmdline ) as $cmd )
            {
                ...
                if ( $cmd )
                {
                    if ( $cmd == '|' ) {
                        ...
                        // insert block separator
                        $line[] = new dsRTECommandSuperSeparator( '', '', '', '', 0 );
                    } elseif ( $cmd == '<|' ) {
                        // insert block separator
                        $line[] = new dsRTECommandLeftSeparator( '', '', '', '', 0 );
                    } elseif ( $cmd == '|>' ) {
                        // insert block separator
                        $line[] = new dsRTECommandRightSeparator( '', '', '', '', 0 );
                    } else {
    ...
    {
        $html = '<table cellspacing="0" cellpadding="0" class="rte" width="100%">';
        $html .= '<tr><td class="rte-cmd" id="'.$this->id.'-cmd">';
        $divs = "";
        // create command's HTML
        foreach ( $this->commands as $lines )
        {
            $html .= '<div class="rte-cmd-toolbar" style="clear:both">';
        ...
        $html .= '<tr><td><div class="dsrte-container"><iframe id="'.$this->id.'" frameborder="0" marginwidth="0" marginheight="0" style="border:0" width="100%"></iframe></div></td></tr>';

        // should this editor be resizable?
        if ( $this->resizable )
            $html .= '<tr><td style="border-top:1px solid #cccccc"><div class="rte-resize" id="'.$this->id.'-resize"><img alt="" src="../common/html/res/dsrte/images/resize.gif" /></div></td></tr>';
    ...
    public function getScripts()
    {
        ...
        if ( self::$notAddedYet )
        {
            ...
            $scripts[] = '<script type="text/javascript" src="../common/html/res/dsrte/dsrte.js"></script>';

            // check for resizable editor
            if ( $this->resizable )
                $scripts[] = '<script type="text/javascript" src="../common/html/res/dsrte/dsrte.resizer.js"></script>';
    ...
    private static function buttonFactory( $id, $name )
    {
        switch ( $name )
        {
            ...
            case 'font':
                $button = new dsRTECommandDivSelect( $id, 'fontname', array('Arial', 'Times', 'Verdana', 'Helvetica', 'Sans'), t( 'Font' ), 25 );
                break;

            case 'fontsize':
                $button = new dsRTECommandDivSelect( $id, 'fontsize', array('2', '3', '5', '7'), t( 'Size' ),  26);
                break;

            case 'headings':
                $button = new dsRTECommandDivSelect( $id, 'formatBlock', array('h1', 'h2', 'h3', 'h4', 'h5', 'h6'), t( 'Headings' ), 27 );
                break;
            ...
            case 'insertvar':
                $button = new dsRTEInsertVarPlugin( $id, 'insertvar', 'insertvar', _s( 'Insert variable' ), 24 );
                break;
...
foreach ( glob( DSRTE_DIR . '/plugins/*.php' ) as $plugin ) {
    require_once( $plugin );
}
