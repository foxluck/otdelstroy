...
class MailParsers
{
	...
	public static function parseAddressString($addr, $addName=true)
	{
		if(!trim($addr))
			return false;
	...
	protected static function _cut_it($str, &$pr, &$em)
	{
		...
		if(($pr=='') && ($em=='') && ($str!='')) // empty part is cutted
			$str = self::_cut_it($str, $pr, $em); // call ones more!
	...
	{
		$str = self::html_decode($str);
		$str = self::cut_scripts($str);

		$str = preg_replace("/\r/", '', $str);

		// delete some tags...
		$str = preg_replace("/<head[^>]*>.*?<\s*\/\s*head>/is", "", $str);
	...
	function formatLead($str)
	{
		$str = preg_replace('/<head[^>]*>.*?<\s*\/\s*head>/is', '', $str);
		$str = preg_replace('/<style[^>]*>.*?<\s*\/\s*style>/is', '', $str);
		$str = preg_replace('/<(br|\/?p|\/?div|\/?tr|ul|li)[^>]*>/i', ' ', $str);
